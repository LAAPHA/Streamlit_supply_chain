...
pages=["Introduction", "Préparation des données", "Data Visualization", "Modélisation"]
...
#   st.write("### Introduction")
  ...
  st.markdown("<p style='text-align: left;'>La supply chain représente les étapes d'approvisionnement allant du processus productif à la distribution de la marchandise au client.<br>  Suite à ces différentes étapes, la satisfaction client est évaluée afin de : <br> -	Étudier la qualité de la supply chain (ex : problème de conception, livraison, prix non adapté, durabilité…).<br> -	Étudier si le produit/service correspond bien à l’attente du marché.<br>  -	Synthétiser les feedback, améliorations des clients.<br>  -	Aider à la réponse ou à la redirection des clients insatisfaits...<br>  Pour de nombreux produits/services, la satisfaction des clients se mesure grâce aux commentaires, et avis laissés par les clients sur des sites dédiées (ex : Trustpilot).</p>", unsafe_allow_html=True)

  st.markdown("<h2 style='text-align: left;'> Objectifs:</h2>", unsafe_allow_html=True)

  st.markdown("<p style='text-align: left;'>L’objectif de ce projet est d’extraire de l’information de commentaires laissés par les clients.<br> Dans un premier temps, l’objectif sera de prédire la satisfaction d’un client à partir des commentaires laissés, c'est-à-dire de prédire le nombre d'étoiles ou la note donnée à partir des commentaires. <br>Puis dans un second temps, l’objectif sera d’extraire les propos du commentaire (problème de livraison, article défectueux...) afin d’expliquer la note attribuée. <br> Enfin, l’objectif sera d’extraire de la réponse du fournisseur les propos du commentaire dans le but d’essayer de les prédire uniquement avec le commentaire afin de générer des réponses automatiques. </p>", unsafe_allow_html=True)
...
# if page == pages[4] :
if page == 4:   
  st.write("### Machine Learning test scrapping")
